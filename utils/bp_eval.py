...
def body_part_association_evaluation(json_path_body, json_path_part, data_cfg, given_body_part=None):
    ...
    if data_cfg['dataset'] == "HumanParts":
        ...
        if given_body_part is None:   
            mMR_list = compute_MMR(json_path_body, val_bfjformat_path, body_part='head')
        else:  # may be ['head', 'face', 'lhand', 'rhand', 'lfoot', 'rfoot']
            mMR_list = compute_MMR(json_path_body, val_bfjformat_path, body_part=given_body_part)
        # mMR_avg = sum(mMR_list) / len(mMR_list)  # "Reasonable", "Small", "Heavy", "All"
        mMR_avg, cnt = 0, 0
        for mMR in mMR_list:
            if mMR > 0:
