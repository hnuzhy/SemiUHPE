...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def collate_fn(batch):
        ...
        index = 0  # image id should increment by consecutive integers
        for i, l in enumerate(label):
            if l.shape[0] == 0:  # this image has no label any more
                continue
            else:
                l[:, 0] = index  # add target image index for build_targets()
                index += 1  # image id should increment by consecutive integers
                new_img.append(img[i])
                new_label.append(l)
                new_path.append(path[i])
                new_shapes.append(shapes[i])
        return torch.stack(new_img, 0), torch.cat(new_label, 0), new_path, new_shapes
