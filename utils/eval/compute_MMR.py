...
class COCOMReval:
    ...
    def __init__(self, cocoGt=None, cocoDt=None, iouType='segm', body_part='face'):
        '''
        ...
        for gt in gts:
            ...
            if self.body_part == 'face':
                gt['ignore'] = 1 if (gt['height'] < self.params.HtRng[id_setup][0] or \
                    ...
                    gt['vis_ratio'] > self.params.VisRng[id_setup][1]) or \
                    ( gt['f_bbox'][3] < self.params.F_HtRng[id_setup][0] or \
                    gt['f_bbox'][3] > self.params.F_HtRng[id_setup][1]) else gt['ignore']
            if self.body_part == 'head':
                gt['ignore'] = 1 if (gt['height'] < self.params.HtRng[id_setup][0] or \
                    gt['height'] > self.params.HtRng[id_setup][1]) or \
                    ( gt['vis_ratio'] < self.params.VisRng[id_setup][0] or \
                    gt['vis_ratio'] > self.params.VisRng[id_setup][1]) or \
                    ( gt['h_bbox'][3] < self.params.F_HtRng[id_setup][0] or \
                    gt['h_bbox'][3] > self.params.F_HtRng[id_setup][1]) else gt['ignore']

            if self.body_part == 'lhand':
                gt['ignore'] = 1 if (gt['height'] < self.params.HtRng[id_setup][0] or \
                    gt['height'] > self.params.HtRng[id_setup][1]) or \
                    ( gt['vis_ratio'] < self.params.VisRng[id_setup][0] or \
                    gt['vis_ratio'] > self.params.VisRng[id_setup][1]) or \
                    ( gt['lh_bbox'][3] < self.params.F_HtRng[id_setup][0] or \
                    gt['lh_bbox'][3] > self.params.F_HtRng[id_setup][1]) else gt['ignore']
            if self.body_part == 'rhand':
                gt['ignore'] = 1 if (gt['height'] < self.params.HtRng[id_setup][0] or \
                    gt['height'] > self.params.HtRng[id_setup][1]) or \
                    ( gt['vis_ratio'] < self.params.VisRng[id_setup][0] or \
                    gt['vis_ratio'] > self.params.VisRng[id_setup][1]) or \
                    ( gt['rh_bbox'][3] < self.params.F_HtRng[id_setup][0] or \
                    gt['rh_bbox'][3] > self.params.F_HtRng[id_setup][1]) else gt['ignore']
            if self.body_part == 'lfoot':
                gt['ignore'] = 1 if (gt['height'] < self.params.HtRng[id_setup][0] or \
                    gt['height'] > self.params.HtRng[id_setup][1]) or \
                    ( gt['vis_ratio'] < self.params.VisRng[id_setup][0] or \
                    gt['vis_ratio'] > self.params.VisRng[id_setup][1]) or \
                    ( gt['lf_bbox'][3] < self.params.F_HtRng[id_setup][0] or \
                    gt['lf_bbox'][3] > self.params.F_HtRng[id_setup][1]) else gt['ignore']
            if self.body_part == 'rfoot':
                gt['ignore'] = 1 if (gt['height'] < self.params.HtRng[id_setup][0] or \
                    gt['height'] > self.params.HtRng[id_setup][1]) or \
                    ( gt['vis_ratio'] < self.params.VisRng[id_setup][0] or \
                    gt['vis_ratio'] > self.params.VisRng[id_setup][1]) or \
                    ( gt['rf_bbox'][3] < self.params.F_HtRng[id_setup][0] or \
                    gt['rf_bbox'][3] > self.params.F_HtRng[id_setup][1]) else gt['ignore']
    ...
    def computeIoU(self, gt, dt):
        ...
        if p.iouType == 'segm':
            ...
            if self.body_part == 'lhand': g_f = [g['lh_bbox'] for g in gt]
            if self.body_part == 'rhand': g_f = [g['rh_bbox'] for g in gt]
            if self.body_part == 'lfoot': g_f = [g['lf_bbox'] for g in gt]
            if self.body_part == 'rfoot': g_f = [g['rf_bbox'] for g in gt]
...
def compute_MMR(json_result_file, anno_file, body_part='face'):
    mr_res_dict = OrderedDict()
    iou_type="bbox"
    id_c = 0
    mr_mode = "crowdhuman"
    ...
    for id_setup in range(4):
        ...
        imgIds = sorted(coco_mod_gt.getImgIds())
        # imgIds = sorted(iduse)
        coco_mr_eval = COCOMReval(coco_mod_gt, coco_mod_dt, iou_type, body_part)
        coco_mr_eval.params.imgIds = imgIds
        coco_mr_eval.evaluate(id_setup)
        coco_mr_eval.accumulate()      
        mean_s = coco_mr_eval.summarize(id_setup) 
